-- Complete security fixes for remaining vulnerabilities (clean version)

-- 1. Fix audit log access - only service role should access
...
-- 3. Fix all remaining functions with mutable search paths
...
-- 4. Remove the duplicate validate_share_access function with 2 parameters
-- Keep only the secure version with 3 parameters
DROP FUNCTION IF EXISTS public.validate_share_access(text, text);

-- 5. Secure storage policies for chat uploads
